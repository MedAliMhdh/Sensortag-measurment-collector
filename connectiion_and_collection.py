...
class _paireddevice():
    ...
    def report(self, tag, value=None):
        ...
        if is_float(value):
            reading = '{"deviceuid":"' + self.addr + '","devicename":"' + self.friendlyname + '","' + tag + '":' + str(
                value) + '}'
        else:
            if not isinstance(value, basestring):
                # a lit of numbers
                #                print "["+",".join([str(x) for x in value])+"]"
                reading = '{"deviceuid":"' + self.addr + '","devicename":"' + self.friendlyname + '","' + tag + '":' + "[" + ",".join(
...
class ScanDelegate(bluepy.btle.DefaultDelegate):
    ...
    def handleDiscovery(self, dev, isNewDev, isNewData):
        ...
        if isNewDev:
            ...
            if args.only:
                if args.info:
                    print
                    "only", dev.addr, devicenames
                if dev.addr not in devicenames:
                    # ignore it!
                    if args.info:
                        print
                        "ignoring only", dev.addr
                    return
            if len(self.activedevlist) < MAXDEVICES:
                ...
                if args.info:
                    print
                    "activedevlist=", self.activedevlist
            else:
                if args.info:
                    print
                    "TOO MANY DEVICES - IGNORED", dev.addr
    ...
    def shutdown(self):
        if args.info:
            print
            "My activedevlist=", self.activedevlist
...
parser.add_argument("-n", "--name",
                    help="basename for default device names if uid not specified in -d option (only used if -o not specified) default is 'ST-' followed by a digit")
parser.add_argument("-f", "--fast", nargs='*', choices=SENSORTYPES,
                    help="list of sensors for fast read rate - defaults to fast read of accelerometer and lightmeter if none of -f/-m/-s are specified")
parser.add_argument("-m", "--medium", nargs='*', choices=SENSORTYPES,
                    help="list of sensors for medium read rate - defaults to fast read of accelerometer and lightmeter if none of -f/-m/-s are specified")
parser.add_argument("-s", "--slow", nargs='*', choices=SENSORTYPES,
                    help="list of sensors for slow read rate - defaults to fast read of accelerometer and lightmeter if none of -f/-m/-s are specified")
...
try:
    while True:
        try:
            ...
            if args.info:
                print
                "Aargh BTLE execption. Not panicing. Carrying on."
